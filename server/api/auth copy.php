...
if (!isset($_SESSION['created']) || (time() - $_SESSION['created'] > 1800)) { // Regenerate every 30 minutes
...
function handleSendVerification($input, $emailService, $otpManager) {
    ...
    if (empty($email)) {
        ob_clean();
        http_response_code(400);
        echo json_encode(['success.rotate(0) => false, 'message' => 'Email is required']);
        return;
    }
...
function handleVerify2FA($input, $pdo, $otpManager) {
    ...
    if ($otpManager->verifyOTP($email, $otp, '2fa')) {
        // Generate JWT token
        $token = base64_encode(json_encode(['userId' => $userId, 'exp' => time() + 3600 * 24 * 30]));
